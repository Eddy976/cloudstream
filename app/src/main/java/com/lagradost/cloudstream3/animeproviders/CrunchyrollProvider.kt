...
class KrunchyProvider : MainAPI() {
    companion object {
        ...
        val episodeNumRegex = Regex("""Épisode (\d+)""")
    }
    val versionFR ="/fr"
    ...
    override val mainPage = mainPageOf(
        "$mainUrl/fr/videos/anime/popular/ajax_page?pg=" to "Populaire",
        "$mainUrl/fr/videos/anime/simulcasts/ajax_page" to "Simulcasts"
    )
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        if (page <= 1 && request.name == "Populaire") {
            val doc = Jsoup.parse(crUnblock.geoBypassRequest(mainUrl+versionFR).text)
            val featured = doc.select(".js-featured-show-list > li").mapNotNull { anime ->
                val url =
                    fixUrlNull(anime?.selectFirst("a")?.attr("href"))
            ...
            val recent =
                doc.select("div.welcome-countdown-day:contains(Disponible maintenant) li")
                    .mapNotNull {
                        val link =
                        ...
                        val dubstat =
                            if (name.contains("Dub)", true)) EnumSet.of(DubStatus.Dubbed) else
            ...
            if (featured.isNotEmpty()) {
                items.add(HomePageList("Séries à l'affiche", featured))
            }
        }
        ...
        if (paginated || !paginated && page <= 1) {
            crUnblock.geoBypassRequest(pagedLink).let { respText ->
                ...
                val episodes = soup.select("li").mapNotNull {
                    ...
                    val urlEps = fixUrlNull(innerA.attr("href"))
                        ?: return@mapNotNull null
                    AnimeSearchResponse(
    ...
    override suspend fun search(query: String): ArrayList<SearchResponse> {
        val json =
            crUnblock.geoBypassRequest("$mainUrl$versionFR/ajax/?req=RpcApiSearch_GetSearchCandidates").text.split(
                "*/"
            )[0].replace("\\/", "/")
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val soup0 = Jsoup.parse(crUnblock.geoBypassRequest(url).text)
        val title = soup0.selectFirst("#showview-content-header .ellipsis")?.text()?.trim()
        val posterU = soup0.selectFirst(".poster")?.attr("src")

        val p = soup0.selectFirst(".description")
        var description = p?.selectFirst(".more")?.text()?.trim()
        if (description.isNullOrBlank()) {
            description = p?.selectFirst("span")?.text()?.trim()
        }

        val genres = soup0.select(".large-margin-bottom > ul:nth-child(2) li:nth-child(2) a")
            .map { it.text().capitalize() }
        val year = genres.filter { it.toIntOrNull() != null }.map { it.toInt() }.sortedBy { it }
            .getOrNull(0)
        val soup = Jsoup.parse(crUnblock.geoBypassRequest(url.replace("$versionFR", "")).text)
    ...
    data class Streams(
        ...
        fun title(): String {
            return when {
                ...
                this.hardsubLang == "frFR" && this.audioLang == "jaJP" -> "Sous-titré (Français)"
                this.audioLang == "esLA" -> "Latino"
                this.audioLang == "esES" -> "Español España"
                this.audioLang == "enUS" -> "English (US)"
                this.audioLang == "frFR" -> "Français (FR)"
                else -> "RAW"
            }
        }
    }
    ...
    override suspend fun loadLinks(
        ...
        //val response = crUnblock.geoBypassRequest(data.replace(versionFR,""))
...
//                        && URI(stream.url).path.endsWith(".m3u")
                    ...
                    // Premium eps
                        ...
                        (listOf("esLA", "esES", "enUS", "frFR", null).contains(stream.hardsubLang))
                    ) {
                        stream.title = stream.title()
                        streams.add(stream)
                    }
            ...
            json.subtitles.forEach {
                val langclean = it.language.replace("esLA", "Spanish")
                    ...
                    .replace("esES", "Spanish (Spain)").replace("frFR", "Français (France)")
                subtitleCallback(
